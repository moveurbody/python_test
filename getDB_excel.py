...
try:
    ...
    for tag in TagList:
        sqlStr = "SELECT COUNT(tag) AS 'Count'\
                 FROM [KernelInfo].[dbo].[AndLogDetail]\
                 WHERE tag='"+tag+"' and CreatedTime BETWEEN DATEADD(Hour, -3, GETDATE()) AND GETDATE()\
                 GROUp BY tag"
    ...
    if ErrorFound:
        ...
        mail.send_mail("yuhsuan_chen@htc.com","yuhsuan_chen@htc.com;a_gangan@hotmail.com","test","test",fullpath)
    else:
        logging.info("Normal!")
    logging.info("--- Check status")
